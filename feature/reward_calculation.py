...
def calculate_reward(step_info, next_step_info):
    ...
    prev_player = step_info['players'][0]
    next_player = next_step_info['players'][0]
    # just base reward info on dist from center
    target_x = 146.0
    target_y = 5.9
    target_z = 19.7
    prev_dist = ((prev_player["position"]["x"] - target_x) ** 2 +
                 (prev_player["position"]["y"] - target_y) ** 2 +
                 (prev_player["position"]["z"] - target_z) ** 2) ** 0.5
    next_dist = ((next_player["position"]["x"] - target_x) ** 2 +
                 (next_player["position"]["y"] - target_y) ** 2 +
                  (next_player["position"]["z"] - target_z) ** 2) ** 0.5
