...
def graph_loss():
    ...
    '''
    log_file = "backups/5-29-Evening/loss_log.txt"
    total_losses = []
    actor_losses = []
    critic_losses = []
    entropy_losses = []
    ...
    plt.xlabel("Chunk")
    plt.ylabel("Loss")
    plt.title("Total Loss Over Chunks")
    plt.legend()
...
def graph_reward():
    ...
    '''
    log_file = "backups/5-29-Evening/training_log.txt"
    episodes = []
    rewards = {}
    total_rewards = []
    normalized_rewards = []
    with open(log_file, "r") as file:
        for line in file:
            ...
            normal_reward = parts[2]
            normal_reward_value = float(normal_reward.split(": ")[1])
            normalized_rewards.append(normal_reward_value)

            episode_part = parts[0]
            episode_num = int(episode_part.split(" ")[1])

            reward_details = parts[3]
